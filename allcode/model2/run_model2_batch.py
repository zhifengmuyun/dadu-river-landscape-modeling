#!/usr/bin/env python3
"""
Batch runner for the 3 x 3 x 3 model2 Badlands experiments.

For each peak_rate_h / peak_rate_v pair this script:
1. regenerates fault displacement CSV files with dlsf matching xshf/anhf geometry,
2. runs uplift_with_noise.py with the requested peak rates,
3. generates newtec1.csv/newtec2.csv/newtec3.csv using the same arithmetic as model2tec.m,
4. copies the tectonic CSV files into this model2 folder,
5. updates Daduhejif.xml for erodibility and output folder name,
6. runs Dadu_run.py in the current Linux/VSCode environment.
"""
...
PEAK_RATE_H = [(1, 0.05), (2, 0.15), (3, 0.25)]
PEAK_RATE_V = [(1, 0.22), (2, 0.69), (3, 1.16)]
...
print("Saved fault displacement CSV files for model2")
'''
    return source[:cut] + save_block


def run_fault_script(settings_dir: Path, dry_run: bool = False) -> None:
    source_path = settings_dir / "fault_displacement_v4.py"
    original = source_path.read_text(encoding="utf-8")
    batch_path = settings_dir / "_fault_displacement_v4_model2_batch.py"
...
def run_uplift_script(settings_dir: Path, peak_h: float, peak_v: float, dry_run: bool = False) -> None:
    source_path = settings_dir / "uplift_with_noise.py"
    original = source_path.read_text(encoding="utf-8")
    batch_path = settings_dir / "_uplift_with_noise_batch.py"
...
def generate_model2_tectonics(settings_dir: Path) -> None:
    nx = 200
    ny = 1000
    n = nx * ny
    t = 1e6
    ...
    gongga_uplift = read_column(settings_dir / "uplift_v_arm_200k.csv") / 1e3 * t
    tuowu_uplift = read_column(settings_dir / "uplift_h_arm_200k.csv") / 1e3 * t
    ...
    disp2 = np.column_stack(
        (dispx_xshf + dispx_anhf, dispy_xshf + dispy_anhf, dispz1 + gongga_uplift)
    )
    disp3 = np.column_stack(
        (
            dispx_xshf + dispx_anhf + dispx_dlsf,
            dispy_xshf + dispy_anhf + dispy_dlsf,
            dispz1 + tuowu_uplift + gongga_uplift,
        )
    )
...
def copy_inputs_to_model(settings_dir: Path, model_dir: Path) -> None:
    for name in (
        "newtec1.csv",
        "newtec2.csv",
        "newtec3.csv",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the 27 model2 uplift/erodibility Badlands experiments."
    )
    parser.add_argument(
        "--settings-dir",
        type=Path,
        help="Folder containing fault_displacement_v4.py and uplift_with_noise.py. Default: model-dir.",
    )
    ...
    parser.add_argument("--overwrite", action="store_true", help="Delete existing model2_x-y-z outputs.")
    parser.add_argument("--skip-existing", action="store_true", help="Skip model2_x-y-z outputs that exist.")
...
def main() -> int:
    ...
    try:
        ...
        run_fault_script(settings_dir, dry_run=args.dry_run)

        for h_index, peak_h in PEAK_RATE_H:
            for v_index, peak_v in PEAK_RATE_V:
                print(
                    f"\n=== Uplift group {h_index}-{v_index}: "
                    f"peak_rate_h={peak_h:g}, peak_rate_v={peak_v:g} ==="
                )
                run_uplift_script(settings_dir, peak_h, peak_v, dry_run=args.dry_run)
                if not args.dry_run:
                    generate_model2_tectonics(settings_dir)
                    copy_inputs_to_model(settings_dir, model_dir)

                for erod_index, erodibility in ERODIBILITIES:
                    out_name = f"model2_{h_index}-{v_index}-{erod_index}"
                    ...
                    print(
                        f"\n--- Run {out_name}: peak_rate_h={peak_h:g}, "
                        f"peak_rate_v={peak_v:g}, erodibility={erodibility} ---"
                    )
    ...
    print("\nAll requested model2 runs are complete.")
    return 0
