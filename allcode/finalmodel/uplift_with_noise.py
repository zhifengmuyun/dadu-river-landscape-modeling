...
sigma_uplift_v = 15
sigma_uplift_h = 100           # 隆升带宽度参数 (km)
peak_rate_v = 0.3           # 最大隆升速率 (mm/a)
peak_rate_h = 0.05
sigma_v_y400 = 25
sigma_v_y500 = 15
extra_peak_rate = 0.05       # 额外贡嘎二维高斯山峰最大隆升速率 (mm/a)
extra_peak_x = 80            # 额外山峰中心 x 坐标 (km)
extra_peak_y = 450           # 额外山峰中心 y 坐标 (km)
extra_peak_sigma_x = 8       # 额外山峰 x 方向宽度 (km)
extra_peak_sigma_y = 12      # 额外山峰 y 方向宽度 (km)

# 生成坐标网格 (1000行 x 200列)
x_coords = np.arange(width)
y_coords = np.arange(height)
X, Y = np.meshgrid(x_coords, y_coords)
sigma_v_field = np.where(
      Y <= 400,
      sigma_v_y400,
      np.where(
          Y >= 500,
          sigma_v_y500,
          sigma_v_y400 - (Y - 400) * (sigma_v_y400 - sigma_v_y500) / (500 - 400)
      )
  )
...
# ===========================
# 基础高斯隆升
W_vertical_arm = peak_rate_v * np.exp(-(dist_v**2 / (2 * sigma_v_field**2)))
W_horizontal_arm = peak_rate_h * np.exp(-(dist_h**2 / (2 * sigma_uplift_h**2)))

# 额外贡嘎山峰：二维高斯隆升，叠加到纵向臂/贡嘎隆升中
W_extra_peak = extra_peak_rate * np.exp(
    -0.5 * (
        ((X - extra_peak_x) / extra_peak_sigma_x) ** 2
        + ((Y - extra_peak_y) / extra_peak_sigma_y) ** 2
    )
)
W_vertical_arm = W_vertical_arm + W_extra_peak

# ===== 横向臂空间限制 =====
# 只保留东侧一半（x > x_cutoff）
x_cutoff = 120  # 截断位置（km），只保留 x > 100 的部分
